...
class Day4 extends Component {
    ...
    componentDidMount(){
        axios.get('https://api.openweathermap.org/data/2.5/forecast?q=München,DE&units=metric&appid=b606f9db39d148829c9500a2651c63dd').then(
            response=>{
                this.setState({DateAndTime:response.data.list[24].dt_txt})
                this.setState({MaxTemp:response.data.list[24].main.temp_max})
                this.setState({MinTemp:response.data.list[24].main.temp_min})
                this.setState({Humidity:response.data.list[24].main.humidity})
                if(response.data.list[24].rain){
                    this.setState({Precipitation:response.data.list[24].main.temp_max})
                }
                else{
    ...
    render(){
        if(this.state.MaxTemp!==null && this.state.MinTemp!==null && this.state.Humidity!==null && this.state.Precipitation!==null && this.state.DateAndTime!==null){
            ...
            <WiDaySunnyOvercast size={100} color='rgb(156, 231, 231)' />
            <h1>Day4 {this.state.DateAndTime}</h1>
